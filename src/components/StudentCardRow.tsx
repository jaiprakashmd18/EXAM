...
function EditableCell({
  value, onSave, numeric, className = "",
}: {
  value: string | number;
  ...
  if (editing) return (
    <input ref={inputRef}
      className={`border border-blue-400 rounded px-1.5 py-0.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-300 bg-blue-50 ${numeric ? "w-14 text-center" : "w-full"} ${className}`}
  ...
  return (
    <>
      <tr className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
        {/* chevron */}
        <td className="pl-4 pr-1 py-4 align-middle w-9">
        ...
        {/* blue icon */}
        <td className="pr-2 py-4 align-middle w-9">
        ...
        {/* professor — editable, no min-width to stay compact */}
        <td className="py-4 pr-3 align-middle whitespace-nowrap">
          <div className="flex items-center gap-1">
            <EditableCell value={course.professor} onSave={(v) => onChange(course.id, "professor", v)}
              className="text-sm text-gray-600" />
            <svg width="13" height="13" viewBox="0 0 24 24" fill="#f97316" className="flex-shrink-0">
        ...
        {/* credit — right-aligned, tight */}
        <td className="py-4 pr-4 align-middle w-16 text-right">
          <span className="text-sm font-semibold text-gray-800">{course.credit}</span>
        </td>
        {/* score — editable */}
        <td className="py-4 pr-4 align-middle w-16 text-right">
          <EditableCell value={course.score} onSave={(v) => onChange(course.id, "score", v)}
            numeric className="text-sm font-semibold text-gray-800" />
        </td>
        {/* grade badge */}
      ...
      {open && (
        <tr className="bg-gray-50 border-b border-gray-100">
          <td colSpan={7} className="py-3 pl-20 pr-4">
